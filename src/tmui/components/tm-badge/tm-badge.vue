<template>
  ...
  <view class=" flex relative" :class="[props.status?'flex-row flex-row-center-center mx-8':'']">
    ...
    <view v-if="show" :class="[
			(_dot||_count||_icon)&&!props.status?'absolute flex-top-start-end':'',
			`r-${offset[0]}`,
			`t-${offset[1]}`,
		]" :style="{zIndex: 10}">
      <tm-sheet
      ...
      >
        <tm-text color="white" :font-size="props.fontSize" :_class="size.h==0?'py-3 px-8':''" v-if="_count>0&&!istext"
                 :label="_count>props.maxCount?props.maxCount+'+':_count"></tm-text>
        <tm-text color="white" :font-size="props.fontSize" :_class="size.h==0?'py-3 px-8':''" v-if="_count&&istext"
                 :label="_count"></tm-text>
