...
<template>
	<view
		...
		:class="['flex flex-col noNvueBorder', parenClass_p, !isDisabledRoundAndriod ? _round : '']"
		:style="[
			{
	...
	>
		<view :class="['flex noNvueBorder flex-col flex-1', customClass]" :style="contStyle_p">
			<slot></slot>
		</view>
	</view>
</template>
...
const tmcomputed = computed<cssstyle>(() => {
	...
	const customPropsDefault = uni.$tm.u.deepObjectMerge(_props_rs, {
		blur: _blur.value,
		text: text
	})
	return computedTheme(customPropsDefault, isDark.value, tmcfg.value)
})
